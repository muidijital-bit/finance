...
import { formatCurrency, CATEGORY_LABELS, SERVICE_LABELS, MONTHS_TR } from '@/lib/utils';
import { PaymentSchedule, TransactionCategory, MuiService } from '@/types';
...
export default function CalendarPage() {
  ...
  // Bu ay takvimde göster - günlere göre sırala
  const byDay = useMemo(() => {
  ...
  function openEdit(p: PaymentSchedule) {
    setForm({ title: p.title, amount: p.amount, currency: p.currency, dueDay: p.dueDay, type: p.type, category: p.category, service: p.service, isActive: p.isActive, note: p.note });
    setEditingId(p.id);
    setModalOpen(true);
  }
  ...
  function toggleActive(p: PaymentSchedule) {
    updatePaymentSchedule(p.id, { isActive: !p.isActive });
  }
    ...
    <div className="space-y-6">
      ...
      <Card>
            ...
            return (
              <div key={day}
                className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-medium transition-colors
                  ${isToday ? 'text-white' : hasPay ? (isPast ? 'bg-gray-100 dark:bg-gray-800 text-gray-400' : 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400') : 'text-gray-300 dark:text-gray-700'}
                `}
        ...
        <div className="space-y-2">
              ...
              <div key={p.id}
                ...
                <div className="flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-lg flex items-center justify-center text-xs font-bold
                    ${p.type === 'expense' ? 'bg-red-100 dark:bg-red-900/30 text-red-600' : 'bg-green-100 dark:bg-green-900/30 text-green-600'}`}>
                    {p.dueDay}
                  </div>
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      ...
                      {p.service ? ` · ${SERVICE_LABELS[p.service]}` : ''}
                      {p.note ? ` · ${p.note}` : ''}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100">
                    <button onClick={() => toggleActive(p)} className="p-1 text-gray-400 hover:text-brand-500 transition-colors" title={p.isActive ? 'Pasif yap' : 'Aktif yap'}>
          ...
          {byDay.length === 0 && (
            <p className="text-center text-sm text-gray-400 py-8">Ödeme takvimi boş</p>
          )}
        </div>
      </Card>
            ...
            {paymentSchedules.filter((p) => !p.isActive).map((p) => (
              <div key={p.id} className="flex items-center justify-between px-4 py-2.5 rounded-lg border border-dashed border-gray-200 dark:border-gray-800 opacity-50">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => toggleActive(p)} className="p-1 text-gray-400 hover:text-green-500 transition-colors">
                    <ToggleLeft size={18} />
                  </button>
                  <button onClick={() => deletePaymentSchedule(p.id)} className="p-1 text-gray-400 hover:text-red-500 transition-colors">
                    <Trash2 size={14} />
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? 'Ödemeyi Düzenle' : 'Yeni Ödeme Ekle'}>
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="text" placeholder="Sunucu kirası, kredi taksidi..." value={form.title}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" value={form.amount || ''} onChange={(e) => setForm({ ...form, amount: parseFloat(e.target.value) || 0 })} className={INPUT_CLS} />
            </div>
            <div>
          ...
          <div>
            <label className={LABEL_CLS}>Hizmet (opsiyonel)</label>
            <select value={form.service ?? ''} onChange={(e) => setForm({ ...form, service: e.target.value as MuiService || undefined })} className={INPUT_CLS}>
          ...
          <div>
            ...
            <input type="text" placeholder="Örn: Vodafone, Is Bankası kredisi..." value={form.note ?? ''}
